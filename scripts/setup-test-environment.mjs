...
const port = Number(process.env.CLOUDX_TEST_PORT ?? "3002");
if (!Number.isInteger(port) || port < 1 || port > 65535) {
  throw new Error("CLOUDX_TEST_PORT must be an integer between 1 and 65535.");
}
const host = process.env.CLOUDX_TEST_HOST?.trim() || "127.0.0.1";
const envRoot = path.join(repoRoot, `.cloudx-test-${port}`);
...
const brokerPidPath = path.join(envRoot, "broker.pid");
const brokerLogPath = path.join(envRoot, "broker.log");
const managedProcesses = [
  { name: "server", pidPath },
  { name: "terminal broker", pidPath: brokerPidPath }
];
...
function helpText() {
  return [
    `Seed and run the CloudX local test environment on port ${port}.`,
    "",
    "Usage:",
    "  node scripts/setup-test-environment.mjs          Seed data only",
    "  node scripts/setup-test-environment.mjs --start  Seed data and start the broker and server",
    "  node scripts/setup-test-environment.mjs --stop   Stop the managed server and broker",
    "",
    "Environment:",
    "  CLOUDX_TEST_HOST=0.0.0.0 binds the test server to all IPv4 interfaces.",
    "  CLOUDX_TEST_PORT=3002 selects the test server port and data directory.",
    "",
    `Generated files live under .cloudx-test-${port}/.`
  ].join("\n");
}
...
function workspaceWindow(id, name, defaultCwd, layout) {
  return {
    ...
    layout,
    pluginMetadata: {
      automation: { fixture: `test-environment-${port}` }
    },
    createdAt: startedAt,
    updatedAt: startedAt
  };
...
function envScript() {
  const env = { CLOUDX_TEST_HOST: host, CLOUDX_TEST_PORT: String(port), ...serverEnv() };
...
function readme() {
  return [
    `# CloudX test environment for port ${port}`,
    "",
    ...
    "",
    "Commands:",
    `Set \`CLOUDX_TEST_PORT=${port}\` for these commands.`,
    "- `npm run testenv:setup` refreshes seed data.",
    "- `npm run testenv:start` builds the web app, refreshes seed data, and starts the terminal broker and server.",
    "- `npm run testenv:stop` stops the managed server, terminal broker, and terminal processes.",
    "",
    `Bind host: ${host}`,
...
async function startServer() {
  ...
  try {
    const brokerLog = fs.openSync(brokerLogPath, "a");
    try {
      const broker = await startTerminalBroker("apps/server/src/terminal/broker.ts", {
        cwd: repoRoot,
        detached: true,
        env,
        execArgv: ["--import", "tsx"],
        stdio: ["ignore", brokerLog, brokerLog, "ipc"],
        onSpawn(child) { fs.writeFileSync(brokerPidPath, `${child.pid}\n`, "utf8"); }
      });
      broker.unref();
    } finally {
      fs.closeSync(brokerLog);
    }
...
async function stopEnvironment() {
  ...
  for (const managed of managedProcesses) {
    try { await stopManagedProcess(managed); }
    catch (error) { failures.push(error); }
  }
  if (failures.length) throw new AggregateError(failures, "The CloudX test environment did not stop completely.");
}

async function stopManagedProcess({ name, pidPath }) {
  if (!fs.existsSync(pidPath)) {
    console.log(`No managed CloudX test ${name} pid file exists.`);
    return;
  }
  ...
  if (processGroupIsAlive(pid)) {
    throw new Error(`Managed ${name} process group ${pid} did not stop after SIGTERM.`);
  }
  fs.rmSync(pidPath, { force: true });
  console.log(`Stopped managed CloudX test ${name} process group ${pid}.`);
}

function assertManagedProcessesStopped() {
  for (const { name, pidPath } of managedProcesses) {
    if (!fs.existsSync(pidPath)) continue;
    const pid = Number(fs.readFileSync(pidPath, "utf8").trim());
    if (Number.isInteger(pid) && pid > 0 && processGroupIsAlive(pid)) {
      throw new Error(`Managed test ${name} is already running with pid ${pid}. Run npm run testenv:stop first.`);
    }
    fs.rmSync(pidPath, { force: true });
  }
}
...
async function waitForHealth(child) {
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null || child.signalCode !== null) {
      throw new Error(`CloudX test server exited before becoming healthy. See ${logPath}.`);
    }
    try {
...
function printSummary(started) {
  ...
  console.log(started ? `Server log: ${logPath}\nTerminal broker log: ${brokerLogPath}` : "Run npm run testenv:start to build and start the terminal broker and server.");
}
