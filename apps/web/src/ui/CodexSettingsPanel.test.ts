...
async function mount(settings = initial, handler?: HookHandler, strict = false) {
  ...
  roots.push(root);
  const panel = createElement(CodexSettingsPanel, { callHook });
  await act(async () => { root.render(strict ? createElement(StrictMode, {}, panel) : panel); });
  return { container, calls, root, callHook };
}
...
describe("global Codex settings editor", () => {
  ...
  it("ignores the first stale load when Strict Mode mounts the editor again", async () => {
    const first = deferred<CodexGlobalSettings>();
    let reads = 0;
    const { container } = await mount(initial, () => ++reads === 1 ? first.promise : { ...initial, model: "current-model" }, true);
    expect(model(container).value).toBe("current-model");
    await act(async () => { first.resolve({ ...initial, model: "stale-model" }); });
    expect(model(container).value).toBe("current-model");
  });
  ...
  it("does not reload when the App provides a new hook callback", async () => {
    ...
    await act(async () => { root.render(createElement(CodexSettingsPanel, { callHook: nextCallHook })); });
    expect(model(container).value).toBe("unsaved-model");
    expect(calls).toHaveLength(1);
    expect(nextCallHook).not.toHaveBeenCalled();
  });

  it("ignores a late load after the editor is unmounted", async () => {
    const pending = deferred<CodexGlobalSettings>();
    const { container, root } = await mount(initial, () => pending.promise);
