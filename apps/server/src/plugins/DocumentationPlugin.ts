...
export class DocumentationPlugin implements WorkspacePlugin {
  ...
  constructor(
    ...
    this.hooks = [
      ...
      externalHook("documentation.documents.checkRevision", "Check Source Revision", "Read the original source and compare its bytes with retained revisions without changing the archive.", (input) => this.client.checkDocumentRevision(requireString(input.documentId, "documentId"), { allowedRoots: this.pathPolicy.configuredRoots() }), { documentId: { type: "string" } }, ["documentId"], documentationRevisionCheckOutputSchema()),
      externalHook("documentation.documents.refresh", "Refresh Source Revision", "Read the original source and retain a new revision when its bytes changed. Prior revisions remain available.", (input) => this.client.refreshDocument(requireString(input.documentId, "documentId"), { allowedRoots: this.pathPolicy.configuredRoots() }), { documentId: { type: "string" } }, ["documentId"], documentationRevisionCheckOutputSchema()),
