...
describe("Reusable review workers", () => {
  ...
  it.each([[false, false], [true, false], [false, true], [true, true]])("uses the latest available reviewer and retains this request's history (saved newest first: %s, newer checkout retained: %s)", async (newestFirst, retainedNewer) => {
    vi.useFakeTimers({ toFake: ["Date"] });
    vi.setSystemTime("2026-09-09T01:00:00.000Z");
    const f = await completedReview();
    ...
    const retainedDraft = { ...draft, id: randomUUID(), startedAt: "2026-09-09T03:00:00.000Z", body: "Review C", comments: [{ body: "Finding C" }],
      status: "posted" as const, publication: { commentIds: ["published-C"] }, postedAt: "2026-09-09T03:30:00.000Z" };
    const retained = { ...latest, id: randomUUID(), startedAt: retainedDraft.startedAt, draft: retainedDraft, worktreePath: undefined, branch: undefined,
      reviewBaseline: { ...latest.reviewBaseline, reviewId: retainedDraft.id },
      retainedWorkspace: { worktreePath: "/retained-review", retainedPaths: ["review-notes.txt"] } };
    const unrelated = [
    ...
    const sameRequest = [older, latest, ...(retainedNewer ? [retained] : [])];
    if (newestFirst) sameRequest.reverse();
    ...
    expect(f.reports.prepare).toHaveBeenLastCalledWith(next.attemptId, expect.objectContaining({ previousReviews: [f.draft, draft, ...(retainedNewer ? [retainedDraft] : [])] }));
    ...
    if (retainedNewer) {
      const { updatedAt, ...retainedReview } = retained;
      expect(f.stored().find(worker => worker.id === retained.id)).toMatchObject(retainedReview);
    }
    expect(f.provider.postReview).not.toHaveBeenCalled();
    expect(f.runtime.prepareWorkspace).toHaveBeenCalledOnce();
  });
