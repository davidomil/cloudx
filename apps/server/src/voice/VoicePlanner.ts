...
export function buildVoicePrompt(transcript: string, context: Record<string, unknown>): string {
  return [
    ...
    "Workspace context includes windows, per-session standardized plugin voiceContext, visibleText, openFile, currentPath, voiceActions, and history.text. history.text is the tab context file and contains recent terminal output, plugin actions, and prior voice actions. It also includes client.windows and client.panes with exact ids, active window/pane, tab ids, active tab, and approximate visual positions. Use that state before choosing actions.",
    ...
    "For window switching, use workspace-control switch_window with an exact windowId/name from client.windows when possible, or a context phrase when matching by context.",
