...
export function buildVoicePrompt(transcript: string, context: Record<string, unknown>): string {
  return [
    ...
    "Use workspace-control.create_tab for requests to open a new Codex, terminal, or file tab. For 'new Codex pane', set input.targetPluginId 'codex-terminal' and input.newPane true.",
    ...
    "For pane placement, splitDirection 'row' means side-by-side columns and 'column' means stacked rows.",
